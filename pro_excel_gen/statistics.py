"""PRO-EXCEL v1.3.1 — 统计分析模块 (Batch 2: statistics)
...
def _skew_kurtosis(arr: np.ndarray) -> tuple[float, float]:
    ...
    kurtosis = float(np.mean((centered / std) ** 4) - 3)
...
def descriptive_stats(
    ...
    *,
    ...
    for col in cols:
        ...
        if len(values) >= 4:
            skew, kurtosis = _skew_kurtosis(arr)
            result[col]["skew"] = skew
            result[col]["kurtosis"] = kurtosis
...
def correlation_analysis(
    ...
    *,
    ...
    if method == "spearman":
        ...
        ranked = np.array([_rank_values(np.array(row, dtype=float)) for row in arr])
