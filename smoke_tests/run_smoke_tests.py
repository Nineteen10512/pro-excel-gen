...
def test_public_imports() -> None:
    ...
    _assert(pro_excel_gen.__version__ == "1.3.1", "Package version mismatch")
...
def test_dependency_manifest() -> None:
    ...
    for package in ("openpyxl", "XlsxWriter", "matplotlib", "numpy", "pandas", "Pillow", "scikit-learn"):
        _assert(package in requirements, f"requirements.txt missing {package}")
        _assert(package in skill_text, f"SKILL.md missing dependency note for {package}")
...
def test_templates() -> None:
    ...
    _assert(len(templates) >= 7, "Expected expanded template registry")
    ...
    funnel = pro_excel_gen.recommend_template("Need a sales funnel conversion workbook")
    _assert(funnel["name"] == "sales_funnel", "Sales funnel recommendation mismatch")
    gantt = pro_excel_gen.recommend_template("Need a project gantt schedule")
    _assert(gantt["name"] == "project_gantt", "Project Gantt recommendation mismatch")
...
def test_ml_tools() -> None:
    ...
    guide = pro_excel_gen.recommend_ml_task(rows, target_column="c")
    _assert(guide["task"] == "classification", "ML guide should recommend classification")

    if importlib.util.find_spec("sklearn") is None:
        try:
            pro_excel_gen.auto_classify(rows, target_column="c")
        except ImportError as exc:
            _assert("scikit-learn is required" in str(exc), "ML dependency error not clear")
            return
        raise AssertionError("ML helper should require scikit-learn when dependency is missing")
...
def run(out_dir: Path) -> int:
    tests = [
        ...
        ("dependency manifest", test_dependency_manifest),
